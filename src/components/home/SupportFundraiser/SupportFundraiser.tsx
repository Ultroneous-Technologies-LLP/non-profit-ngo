...
export const SupportFundraiser: FC<SupportFundraiserProps> = ({
  ...
  >
    <div className="z-10 md:pt-3 xl:pt-11">
      <h2 className="font-segoe-regular text-primary-blue pb-2 text-center text-[32px]/10 font-semibold xl:pb-2.5 xl:text-6xl/23.5">
        <span>{title}</span>
      </h2>
    ...
    <div className="flex flex-col justify-between gap-8 text-center md:flex-row">
      {fundraisers.map(({ alt, donateText, src, link, subtitle, title: fundraisersTitle, id }) => (
        <div className="mx-auto w-full md:max-w-75 xl:max-w-99" key={id}>
          <Image
            alt={alt}
            className="mx-auto h-60 w-81.5 rounded-3xl md:h-45 md:w-75.5 xl:h-76 xl:w-99 xl:rounded-[20px]"
            height={305}
            src={src}
            width={397}
          />
          ...
          <h4 className="font-segoe-regular pb-4 text-xs/4 font-semibold text-[#8999AA] xl:pb-5 xl:text-2xl/8">
            <span>{subtitle}</span>
          </h4>
          <Link
    ...
    <Image
      ...
      className="absolute top-12! z-0 xl:top-2.5! xl:left-11! xl:h-[374px]! xl:w-[1440px]!"
      fill
